...
In this case, the GE has enough time to fill all four buffers because each frame takes very little time. Therefore, the frame time is limited to a minimum of 2000 microseconds. If the frame time goes too low, it would be necessary to increase the number of back buffers, which could lead to VRAM limitations.
